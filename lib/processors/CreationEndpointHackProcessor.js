/**
 * Copyright (c) KhulnaSoft, Ltd. and its affiliates.
 *
 * @format
 *  strict-local
 */
...
const processor = {
  process(specs, metadata) {
    ...
    const clsWithCreationApi = ['AdAccount', 'Business', 'ProductCatalog', 'Hotel'];
        ...
        if (APISpec.method === 'POST') {
          ...
          if (createdCls && createdCls !== parentClsName) {
            ...
            if (createdClsSpec && !createdClsSpec.exclude_creation_endpoint && creationEndpoint) {
              ...
              if (!createdClsSpec.multi_creation_endpoints || APISpec.preferred_creation_endpoint) {
                ...
                if (APISpec.preferred_creation_endpoint) {
                  createdClsSpec.preferred_creation_endpoint = APISpec.preferred_creation_endpoint;
                }
              }
